function [output_op_info]=ecog_langloc_find_noise_free_electrodes(datafile,sub_info)

try ecog_ch=sub_info.op_info.ecog_channels; catch,ecog_ch=[]; end 
try ref_ch=sub_info.op_info.Ref; catch,ref_ch=[]; end 
try gnd_ch=sub_info.op_info.GND; catch,gnd_ch=[]; end 
try bad_ch=sub_info.op_info.bad_channels; catch,bad_ch=[]; end 
try mic_ch=sub_info.op_info.microphone_channels; catch,mic_ch=[]; end 
try good_run=sub_info.op_info.good_run; catch,good_run=[]; end 
try num_ch=sub_info.op_info.num_channels; catch,num_ch=[]; end 
try seizure_ch=sub_info.op_info.seizure_channels; catch,seizure_ch=[]; end 
...
if unique(SourceCh)~=length(param.channels)
    ...
    try param.channels=[1:num_ch]'; catch, param.channels=[1:(unique(SourceCh))]'; end 
end 
...
session_length=[];
% go through all data files
try valid_files=reshape(find(good_run),1,[]); catch valid_files=1:length(datafile);end 

for idx_file = valid_files
...
for kk=1:kk_total
    ...
    if any(ismember(t_window,session_start))
        ...
        arrayfun(@(x) text(t_window(x),size(x_norm_cell,1)+1,['sess: ',reshape(session_name,1,[])],'VerticalAlignment','bottom','fontsize',20),find(ismember(t_window,session_start)))
    end 
    shg;
...
param.channels_deselect = sort(unique([reshape(param.channels_noise,1,[]),unique([ref_ch,gnd_ch,seizure_ch,bad_ch]) ]));
...
%% add the results to sub_info
sub_info.op_info.channel_removed_by_user=x;
sub_info.op_info.transmit_chan=param.channels;
sub_info.op_info.unselected_channels=param.channels_deselect;
sub_info.op_info.clean_channels=param.channels_selected;
sub_info.op_info.sampling_rate=sampling_rate;
sub_info.op_info.channel_noise_across_all_sess=signal_noise_before;
sub_info.op_info.channel_denoise_across_all_sess=signal_noise_after;
output_op_info=sub_info;
