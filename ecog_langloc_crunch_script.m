...
sub_info_path='/Users/eghbalhosseini/MyData/ecog-langloc/sub_operation_info/';
...
d= dir(strcat(data_path,'/**/',subject_name,'/**',subject_op_info.op_info.task_file_location,'/**/ECOG*.dat'));
d_files=arrayfun(@(x) {strcat(d(x).folder,'/',d(x).name)}, 1:length(d));
fprintf(' %d .dat files were found \n', length(d_files));

%% 
subject_op_info=ecog_langloc_find_noise_free_electrodes(d_files,subject_op_info.([subject_name,'_op']));
eval(strcat(subject_op_info.op_info.subject_name,'_op','=subject_op_info'));

save(strcat(sub_info_path,subject_op_info.op_info.subject_name,'_operation_info.mat'),strcat(subject_op_info.op_info.subject_name,'_op'),'-v7.3');

%% 

try good_run=subject_op_info.op_info.good_run; catch,good_run=[]; end 
try valid_files=reshape(find(good_run),1,[]); catch valid_files=1:length(d_files);end 
for i=valid_files
   ...
   [signal_broadband,...
    ...
    StimDur_indx=cell2mat(cellfun(@(x) strcmp(x,'StimulusDuration'),parameters.Stimuli.RowLabels,'UniformOutput',false));
...
%    
    ...
    for k=1:length(trial_seq_cell)
        ...
        for kk=1:length(trial_indx)
            ...
            if strfind(stimuli_value{StimType_indx,trial_indx(kk)},'word')
                ...
                word_dur=stimuli_value{StimDur_indx,trial_indx(kk)};
            end 
            if strfind(stimuli_value{StimType_indx,trial_indx(kk)},'probe')
                ...
                probe_dur=stimuli_value{StimDur_indx,trial_indx(kk)};
            end
            if strfind(stimuli_value{StimType_indx,trial_indx(kk)},'preprobe')
                preprobe_dur=stimuli_value{StimDur_indx,trial_indx(kk)};
            end
            if strfind(stimuli_value{StimType_indx,trial_indx(kk)},'postprobe')
                postrobe_dur=stimuli_value{StimDur_indx,trial_indx(kk)};
            end
            if strfind(stimuli_value{StimType_indx,trial_indx(kk)},'feedback')
                feedback_dur=stimuli_value{StimDur_indx,trial_indx(kk)};
            end
        ...
        postprobe_duration{k,1}=postrobe_dur;
        preprobe_duration{k,1}=preprobe_dur;
        feedback_duration{k,1}=feedback_dur;
        word_duration{k,1}=word_dur;
    ...
    info.postprobe_duration=postprobe_duration;
    info.preprobe_duration=    preprobe_duration;
    info.feedback_duration=    feedback_duration;
    info.word_duration=    word_duration;
    %
